...
// Pendaftaran Otorisasi Perangkat Terminal Kasir (Bisa diakses untuk registrasi perangkat baru)
Route::get('/kasir/authorize-device', [KasirLoginController::class, 'authorizeDevice'])->name('kasir.authorize-device');

// Proteksi Khusus Terminal Kasir: Hanya Jaringan Wi-Fi/IP Kafe atau Perangkat Terdaftar yang dapat membuka
Route::middleware([RestrictKasirAccess::class])->group(function () {
    ...
    Route::middleware('auth')->prefix('kasir')->name('kasir.')->group(function () {
        ...
        Route::prefix('music')->name('music.')->group(function () {
            ...
            // Ban List Lagu (Manual & Auto Blacklist)
            Route::get('/ban-list', [KasirMusicController::class, 'bannedTracksJson'])->name('ban.list');
            Route::post('/ban-list', [KasirMusicController::class, 'storeBannedTrack'])->name('ban.store');
            Route::delete('/ban-list/{bannedTrack}', [KasirMusicController::class, 'destroyBannedTrack'])->name('ban.destroy');
            Route::patch('/ban-list/{bannedTrack}/toggle', [KasirMusicController::class, 'toggleBannedTrack'])->name('ban.toggle');
            Route::post('/requests/{musicRequest}/ban', [KasirMusicController::class, 'banRequest'])->name('requests.ban');
            Route::post('/ban-list/quick-ban-current', [KasirMusicController::class, 'quickBanCurrentTrack'])->name('ban.current');
